...
CODEXBAR="${CODEXBAR_BIN:-$(command -v codexbar 2>/dev/null || printf '%s' "${HOME}/.local/bin/codexbar")}"
CONFIG_PATH="${XDG_CONFIG_HOME:-${HOME}/.config}/codexbar/config.json"
if [[ ! -f "$CONFIG_PATH" && -f "${HOME}/.codexbar/config.json" ]]; then
    CONFIG_PATH="${HOME}/.codexbar/config.json"
fi
DEEPSEEK_KEY_FILE="${XDG_CONFIG_HOME:-${HOME}/.config}/codexbar/deepseek-api-key"
...
if [[ "$merged" == "[]" ]]; then
    printf '{"text":"󰚩","tooltip":"CodexBar: no provider data","class":"stale","percentage":0}\n'
    exit 0
fi
